...
class SettingsTable(tk.Frame):
    def __init__(self, parent, columns):
        ...
        for i, column in enumerate(self.columns):
            self.grid_columnconfigure(i, weight=column['width'])

        # Create the header row
        for i, column in enumerate(self.columns):
            a = tk.Label(self, text=column['name'])
        ...
        for i in range(6):
            ...
            for j, column in enumerate(self.columns):
                if column["type"] == "bool":
                    ...
                    var = tk.BooleanVar()
                    b = tk.Checkbutton(self, variable=var)
...
class SettingsFrameWidget(tk.Frame):
    def __init__(self, parent):
        columns = [{"name": "Id", "width": 10, "type": "int"},                   # 10% width, integer data
            {"name": "Active", "width": 100, "type": "int"},    
            {"name": "Current sensor address", "width": 100, "type": "str"},
            {"name": "Relays address", "width": 100, "type": "str"},        
                      
        ]
  
        tk.Frame.__init__(self, parent)
        self.label = tk.Label(self, text="Settings frame")
        self.table = SettingsTable(self, columns)
